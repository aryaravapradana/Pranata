...
export function GlobalRope() {
  ...
  const outlineRef =
    useRef<SVGPathElement>(null);
  const [svgHeight, setSvgHeight] =
    useState(0);
  const [svgWidth, setSvgWidth] =
    useState(0);
  const [strokeW, setStrokeW] =
    useState(120);
  ...
  const updateRopeAndAnimation =
    useCallback(() => {
      const pathEl = pathRef.current;
      if (!pathEl) return;
      const wrapper = document.querySelector(
        ".features-rope-container",
      ) as HTMLElement;
      if (!wrapper) return;
      ...
      if (mobile) {
        ...
        if (glowRef.current) {
          glowRef.current.style.strokeDasharray =
            "none";
          glowRef.current.style.strokeDashoffset =
            "0";
        }
        if (outlineRef.current) {
          outlineRef.current.style.strokeDasharray =
            "none";
      ...
      gsap.fromTo(
        ...
        { strokeDashoffset: length },
        {
          ...
          scrollTrigger: {
            trigger: wrapper,
            start: () =>
              `top+=${rawAnchors[0].y}px center`,
            end: () =>
              `top+=${rawAnchors[rawAnchors.length - 1].y}px center`,
            scrub: 0.1,
  ...
  return (
    <div
      ...
      className={cn(
        ...
        "w-full pointer-events-none opacity-100",
        "transform-gpu",
      )}
      style={{
        zIndex: 10,
    ...
    >
      <svg
        ...
        className={cn(
          "absolute top-0 left-0",
          "w-full overflow-visible pointer-events-none",
          "transform-gpu",
        )}
        style={{
          height: `${svgHeight}px`,
