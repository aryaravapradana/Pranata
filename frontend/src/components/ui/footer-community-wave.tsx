...
export function FooterCommunityWave({ variant = "light" }: FooterProps) {
  ...
  const waveFill = isLightPage ? "#32452C" : "#F8F6F0";
  const footerBg = isLightPage ? "bg-[#32452C] text-[#E8E3D2]" : "bg-[#F8F6F0] text-[#1C241E]";
  const logoSrc = isLightPage ? "/logos/basic/logo-white.webp" : "/logos/basic/logo black.webp";
  const subtextColor = isLightPage ? "text-white font-medium opacity-90" : "text-[#5A635B]";
  const linkTextMain = isLightPage ? "text-white font-bold hover:underline hover:opacity-80" : "text-[#1C241E] font-bold hover:text-[#C25939] hover:underline";
  const linkTextSub = isLightPage ? "text-white font-bold hover:underline hover:opacity-80" : "text-[#32452C] font-bold hover:text-[#C25939] hover:underline";
  ...
  const devTitleColor = isLightPage ? "text-white group-hover:text-[#1C241E]" : "text-[#1C241E] group-hover:text-white";
  ...
  return (
    <div className="w-full font-sans overflow-hidden">
      ...
      {/* ── Main Footer Body ── */}
      <footer className={`relative ${footerBg} pt-4 pb-16 sm:pb-20 md:pb-24 px-4 sm:px-8 lg:px-12 z-20 w-full`}>
