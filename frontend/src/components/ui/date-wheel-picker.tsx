...
function WheelColumn({
  ...
  const itemsLengthRef = React.useRef(
    items.length,
  );
  ...
  const handleDragEnd = (
    ...
    const projectedY =
      currentY + velocity * 0.2;

    let newIndex = Math.round(
      -projectedY / itemHeight,
    );
    newIndex = Math.max(
      0,
    ...
    >
      ...
      <div
        className={cn(
          ...
          "pointer-events-none border-y border-[#E8E3D2]",
          "bg-[#B4C179]/10",
        )}
        style={{
          top: centerOffset,
  ...
  (
    {
      ...
      onChange,
      minYear = 1920,
      maxYear = new Date().getFullYear() + 5, // Allow future dates for scheduling
      size = "md",
      disabled = false,
      locale = "id-ID",
      className,
