...
export function SplashScreen() {
  ...
  const isClosed = phase === 'INITIAL' || phase === 'CLOSING' || phase === 'COVERED';
  ...
  const outTransition = {
    duration: 0.95,
    ease: [0.85, 0, 0.15, 1] as import("framer-motion").Easing
  };

  return (
    <div 
      className={`fixed inset-0 z-[99999] transition-opacity duration-300 pointer-events-none ${
        isGlobalReady ? 'opacity-0' : 'opacity-100'
      }`} 
      style={{ contain: "strict", willChange: "opacity, transform" }}
    >
          ...
          <mask id="splash-mask">
            ...
            <svg x="calc(50vw - min(40vw, 200px))" y="calc(50vh - min(11.89vw, 59.45px))" width="min(80vw, 400px)" height="min(23.78vw, 118.9px)" viewBox="0 0 3343 994" overflow="visible">
              <motion.circle
                ...
                fill="black"
                initial={{ r: 0 }}
                animate={{ r: isClosed ? 0 : (targetRadius || 15000) }}
