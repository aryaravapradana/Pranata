...
function WheelItem({
  ...
  const itemY = useTransform(y, (latest) => index * itemHeight + latest);
...
function Column({
  ...
  const snapToClosest = React.useCallback(
    (currentY: number) => {
      const targetIndex = Math.round(-currentY / itemHeight);
    ...
    [itemHeight, items.length, onSelectIndex, y]
  );

  return (
    <div className="flex flex-col items-center flex-1 min-w-[65px]">
      {label && (
        <span className="text-[10px] font-black uppercase text-[#7A8678] tracking-widest mb-1.5 select-none">
          {label}
        </span>
      )}
      <div
      ...
      >
        ...
        <div
          ...
          className="absolute inset-x-1 rounded-xl bg-[#2B4C3B] shadow-md shadow-[#2B4C3B]/20 pointer-events-none z-0"
        />
        ...
        <motion.div
          ...
          onDragEnd={(_, info) => {
            ...
            const currentY = y.get() + info.velocity.y * 0.15;
...
const TimeWheelPicker = React.forwardRef<HTMLDivElement, TimeWheelPickerProps>(
  (
    ...
    const minutes = React.useMemo(() => {
      const step = Math.max(1, Math.min(30, stepMinute));
      const arr: string[] = [];
    ...
    const parsedTime = React.useMemo(() => {
      ...
      let hIdx = hours.indexOf(hStr.padStart(2, "0"));
      if (hIdx === -1) hIdx = 8;

      let mIdx = minutes.indexOf(mStr.padStart(2, "0"));
      if (mIdx === -1) {
        const mVal = parseInt(mStr, 10) || 0;
        let closest = 0;
        let minDiff = 999;
        minutes.forEach((mItem, idx) => {
          const diff = Math.abs(parseInt(mItem, 10) - mVal);
          if (diff < minDiff) {
            minDiff = diff;
            closest = idx;
          }
    ...
    const handleHourSelect = (idx: number) => {
      ...
      const curM = minutes[parsedTime.mIdx] || "00";
      onChange(`${newH}:${curM}`);
    };

    const handleMinuteSelect = (idx: number) => {
      const curH = hours[parsedTime.hIdx] || "08";
    ...
    return (
      ...
      >
        ...
        <div className="flex items-center justify-center w-full mb-3 px-1">
          <div className="flex items-center gap-1.5 text-[#32452C] font-black text-xs">
            <Clock size={14} className="text-[#C25939]" />
            <span>Pilih Jam</span>
          </div>
        </div>

        {/* 3D Wheel Column Picker */}
        <div className={cn("flex items-center justify-center w-full", config.gap)}>
          <Column
            items={hours}
          ...
          <div className="flex items-center justify-center text-[#2B4C3B] font-black text-xl pt-4 animate-pulse">
            :
          </div>
