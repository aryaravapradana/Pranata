...
import { Bot, User, Send, Paperclip, Loader2, Sparkles, X, ArrowUp, ChevronLeft, ShieldCheck, RefreshCw } from "lucide-react";
...
export default function StandaloneIntelligencePage() {
  ...
  return (
    <div className="min-h-screen bg-[#F8F6F0] text-[#1C241E] flex flex-col justify-between selection:bg-[#2B4C3B] selection:text-white">
      {/* ── Top Header Navigation ── */}
      <header className="w-full max-w-6xl mx-auto pt-4 px-4 sm:px-6 flex items-center justify-between z-20 shrink-0">
        <div className="flex items-center gap-3">
          <Link 
            href="/hub" 
            className="w-10 h-10 rounded-full bg-white border border-[#E8E3D2] flex items-center justify-center text-[#1C241E] hover:bg-[#EEF2E6] hover:text-[#2B4C3B] transition-colors shadow-sm active:scale-95 cursor-pointer"
      ...
      {/* ── Main Canvas Content ── */}
      <main className="flex-1 w-full max-w-5xl mx-auto px-4 sm:px-6 py-6 flex flex-col justify-center items-center">
        {messages.length === 0 ? (
          ...
          <div className="w-full flex-1 overflow-y-auto mb-6 space-y-5 px-2 custom-scrollbar max-h-[60vh]">
            <AnimatePresence initial={false}>
              {messages.map(m => (
                ...
                >
                  {m.role === 'assistant' && (
                    <div className="w-9 h-9 sm:w-10 sm:h-10 shrink-0 rounded-2xl bg-gradient-to-br from-[#1C241E] to-[#2B4C3B] flex items-center justify-center text-white shadow-md mt-1 border border-white/20">
                      <Sparkles size={18} className="text-emerald-400" />
                    </div>
                  )}

                  <div className={`max-w-[88%] sm:max-w-[82%] rounded-[2rem] p-4 sm:p-5 shadow-sm ${
                    m.role === 'user' 
            ...
            {isLoading && (
              <div className="flex gap-3 sm:gap-4">
                <div className="w-9 h-9 sm:w-10 sm:h-10 shrink-0 rounded-2xl bg-[#2B4C3B] flex items-center justify-center text-white shadow-md mt-1 border border-white/20 animate-pulse">
        ...
        <div className="w-full max-w-3xl">
          ...
          {files && files.length > 0 && (
            <div className="flex gap-2 mb-3 overflow-x-auto pb-1">
              {Array.from(files).map((file, i) => (
                <div key={i} className="relative bg-white rounded-xl p-2.5 flex items-center gap-2 text-xs font-bold text-[#1C241E] border border-[#E8E3D2] shadow-sm shrink-0">
          ...
          {error && (
            <div className="text-red-600 text-xs font-bold mb-3 p-3 bg-red-50 rounded-2xl border border-red-200">
          ...
          <form 
            onSubmit={onSubmit}
            className="bg-[#1C241E] text-white p-4 sm:p-5 rounded-[2.5rem] shadow-2xl border border-[#2B4C3B]/50 relative overflow-hidden transition-all focus-within:ring-2 focus-within:ring-[#4A7C59]/40"
          >
            {/* Top Sparkle Icon */}
            <div className="flex items-center gap-1.5 mb-2 text-white/70">
              <Sparkles size={16} className="text-[#F5990D]" />
            </div>
            ...
            <textarea
              ref={textareaRef}
              rows={2}
              className="w-full bg-transparent border-none text-white text-sm sm:text-base font-medium focus:outline-none focus:ring-0 resize-none placeholder:text-white/40 leading-relaxed"
            ...
            <div className="flex items-center justify-between pt-3 mt-1 border-t border-white/10">
              ...
              <button 
                ...
                className="inline-flex items-center gap-2 bg-white/10 hover:bg-white/20 text-white text-xs font-extrabold px-3.5 py-2 rounded-full transition-colors border border-white/10 cursor-pointer active:scale-95"
              >
                <Paperclip size={14} className="text-[#F5990D]" />
                <span>Lampirkan File / Gambar</span>
              </button>

              <button 
                type="submit"
                disabled={isLoading || (!input && !files)}
                className="w-10 h-10 sm:w-11 sm:h-11 rounded-full bg-[#2B4C3B] hover:bg-[#3B664C] text-white flex items-center justify-center shadow-lg transition-all disabled:opacity-40 disabled:hover:bg-[#2B4C3B] active:scale-95 cursor-pointer shrink-0"
              >
      ...
      {/* ── Footer ── */}
      <footer className="w-full text-center py-4 text-[11px] font-bold text-[#7A8678] tracking-wider uppercase shrink-0">
