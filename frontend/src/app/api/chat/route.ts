...
export async function POST(req: Request) {
  ...
  if (!apiKey) {
    console.warn(
      "WARNING: GEMINI_API_KEY is not defined in process.env!",
    );
  }
  ...
  const compactEvents = futureEvents
    .slice(0, 6)
    .map((e: any) => ({
      title: e.title,
...
- Kondisi Cuaca Lokasi: ${contextData.weather?.temperature_2m ? `${Math.round(contextData.weather.temperature_2m)}°C, Kelembapan ${contextData.weather.relative_humidity_2m}%` : "Normal"}
  ...
  const coreMessages = recentMessages.map(
    (msg: any) => {
      const role =
        msg.role === "assistant"
          ? "assistant"
      ...
      const promptText =
        ...
        textContent.trim() !== ""
          ? textContent
          : "Tolong analisis foto / lampiran ini.";
      contentParts.push({
        type: "text",
        text: promptText,
      });
        ...
        if (att.url.startsWith("data:")) {
          ...
          if (
            ...
            try {
              ...
              console.error(
                "Failed to parse attachment base64 buffer",
                e,
              );
            }
          }
...
JANGAN GUNAKAN TEKS UMUM SEPERTI "TINGKATKAN PENJUALAN". SEBUTKAN NAMA PRODUK / ANGKA NYATA SESUAI KONTEKS.${dynamicContext}`,
      ...
      console.warn(
        `Model ${modelName} failed or quota exceeded:`,
        error?.message || error,
      );
      lastError = error;
    }
  ...
  console.error(
    "All AI models failed, using smart data-driven local fallback:",
