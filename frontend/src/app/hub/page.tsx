  ...
  const getWeatherDetails = (
    ...
    if (code === 0)
      return {
        ...
        icon: "☀️",
        advice:
          "Cuaca cerah. Sangat baik untuk aktivitas kandang & pengiriman.",
      };
    if ([1, 2].includes(code))
      return {
        ...
        icon: "⛅",
        advice:
          "Cuaca hangat berawan. Pastikan sirkulasi udara kandang lancar.",
      };
    if (code === 3)
      return {
        text: "Berawan",
        icon: "☁️",
        advice:
          "Cuaca teduh berawan. Cocok untuk pemberian pakan & kesehatan ternak.",
      };
    if ([45, 48].includes(code))
      return {
        ...
        icon: "🌫️",
        advice:
          "Jarak pandang terbatas karena kabut. Berhati-hati saat pengiriman.",
      };
    if ([51, 53, 55, 56, 57].includes(code))
      return {
        ...
        icon: "🌦️",
        advice:
          "Gerimis turun. Jaga kelembapan alas ternak tetap kering.",
      };
    if ([61, 63, 65, 66, 67].includes(code))
      return {
        ...
        icon: "🌧️",
        advice:
          "Terjadi hujan. Waspada lantai kandang licin & kelembapan tinggi.",
      };
    if ([80, 81, 82].includes(code))
      return {
        ...
        icon: "🌧️",
        advice:
          "Hujan lebat. Pastikan pakan tersimpan rapat & terlindung dari air.",
      };
    if ([95, 96, 99].includes(code))
      return {
        ...
        icon: "🌩️",
        advice:
          "Waspada badai petir. Pastikan kelistrikan & tirai kandang tertutup aman.",
      };
    return {
      text: "Cerah Berawan",
      icon: "⛅",
      advice:
        "Suhu & kondisi stabil untuk kegiatan pemeliharaan ternak.",
    };
  };
  ...
  const fetchWeatherForCoords = async (
    ...
    try {
      ...
      const weatherRes = await fetch(
        `https://api.open-meteo.com/v1/forecast?latitude=${lat}&longitude=${lng}&current=temperature_2m,relative_humidity_2m,is_day,precipitation,weather_code,wind_speed_10m&timezone=auto`,
      );
      if (weatherRes.ok) {
        const weatherData =
  ...
  useEffect(() => {
    if (
      ...
      const isFallback =
        ...
        lastMsgContent.includes(
          "TITLE: Status Etalase",
        );

      if (!isFallback) {
        localStorage.setItem(
          `pranata_ai_insight_cache_${profile.id}`,
          JSON.stringify({
            timestamp: Date.now(),
            messages: messages,
          }),
    ...
    >
      <div className="w-full mx-auto px-3.5 sm:px-6 md:px-8 pt-2 pb-6">
        ...
        <div className="mb-4">
          ...
          >
            Pusat kendali operasional harian
            Anda.
          </p>
        </div>
        ...
        <div
          className={cn(
            ...
            "lg:grid-cols-3 gap-3.5 sm:gap-4",
          )}
        >
          {/* Weather Widget */}
          {(() => {
                ...
                >
                  <div
                    className={cn(
                      ...
                      "bg-white/20 backdrop-blur-md px-3",
                      "py-1 sm:px-3.5 sm:py-1.5",
                      "rounded-full text-[11px] sm:text-xs",
                      "font-black tracking-wider uppercase",
                      "truncate max-w-[85%] border",
                  ...
                  <button
                    ...
                    className={cn(
                      "p-1.5 bg-white/20 hover:bg-white/30",
                      "rounded-full transition-all backdrop-blur-md",
                      "active:scale-95 text-white disabled:opacity-50",
                    ...
                    <div className="space-y-3 sm:space-y-4">
                      <div className="flex items-baseline justify-between gap-3">
                        <div className="flex items-baseline gap-1.5 sm:gap-2">
                          ...
                          <span className="text-xl sm:text-2xl font-black text-[#B4C179]">
                            C
                          </span>
                        </div>
                        ...
                        <div
                          className={cn(
                            ...
                            "px-3 py-1 sm:px-3.5",
                            "sm:py-1.5 rounded-full backdrop-blur-md",
                            "shadow-xs",
                          )}
                        >
                      ...
                      <div className="grid grid-cols-2 gap-2.5 sm:gap-3 pt-1">
                        <div
                          className={cn(
                            "bg-black/20 backdrop-blur-md p-2.5",
                            "sm:p-3 rounded-xl sm:rounded-2xl",
                        ...
                        >
                          <div
                            className={cn(
                              "p-1.5 sm:p-2 bg-[#4A7C59]/50",
                              "rounded-lg sm:rounded-xl shrink-0",
                            )}
                          >
                            <Droplets
                              size={18}
                          ...
                          <div className="min-w-0">
                            <p
                              className={cn(
                                ...
                                "text-[#A4C4A8] uppercase tracking-wider",
                                "truncate",
                              )}
                            >
                              Kelembapan
          ...
          >
            ...
            >
              {!isLoaded ? (
                  ...
                  .map((order, i) => (
                    <div
                      ...
                      className={cn(
                        ...
                        "hover:bg-white/15 transition-colors shadow-sm",
                        "backdrop-blur-md",
                      )}
                    >
                      <div className="flex-1 min-w-0 pr-2">
                        <div className="flex items-center gap-1.5 mb-0.5">
                          ...
                          <span className="text-[9px] font-bold text-[#A4C4A8] shrink-0">
                            INV-
                            {order.id.substring(
                              0,
                              4,
                      ...
                      <div className="text-right shrink-0">
                        <span
                          className={`text-[9px] font-black px-2 py-0.5 rounded-md block mb-1 w-fit ml-auto ${
                            order.status ===
                            "PENDING"
                              ? "bg-[#F1EBE1] text-[#C25939]"
                              : order.status ===
                                  "PROCESSING"
                                ? "bg-[#EEF2E6] text-[#2B4C3B]"
                                : order.status ===
                                    "SHIPPED"
                                  ? "bg-[#E3F0F4] text-[#246A80]"
                ...
                >
                  ...
                  <p className="text-xs font-bold text-[#A4C4A8]">
            ...
            <div
              className={cn(
                "shrink-0 pt-2.5 sm:pt-3",
                "flex justify-end border-t",
                "border-white/10 mt-1",
              )}
            >
              <Link
                href="/hub/orders"
          ...
          <div
            className={cn(
              "md:col-span-2 lg:col-span-2 order-4",
              "lg:order-4 bg-pranata rounded-3xl",
              "sm:rounded-[2rem] p-4.5 sm:p-6",
          ...
          >
            ...
            <div
              className={cn(
                "order-2 md:order-1 w-full",
                "md:flex-1 flex flex-col",
                "min-w-0 md:h-full pt-2",
                "md:pt-0 border-t md:border-t-0",
                "border-white/15 mt-1 md:mt-0",
              )}
            >
              <div className="shrink-0 mb-2 sm:mb-3">
                <h3
                  className={cn(
                    ...
                    "font-black text-white flex",
                    "items-center gap-2 capitalize",
                  )}
                >
                  <Calendar
                    className="text-[#A4C4A8] shrink-0"
                    size={18}
                  />
                  <span className="truncate">
              ...
              <div
                className={cn(
                  "max-h-32 sm:max-h-36 md:max-h-none",
                  "overflow-y-auto min-h-0 pr-1",
                  "sm:pr-2 space-y-2 pb-1",
                  "hide-scrollbar",
                )}
              >
                {(() => {
                    ...
                    return (
                      <div
                        className={cn(
                          ...
                          "text-[#84B0A5] py-1 md:py-0",
                          "md:h-full flex items-center",
                        )}
                      >
                        Belum ada jadwal
                        mendatang.
                  ...
                  return futureEvents.map(
                    (e, idx) => {
                      ...
                      return (
                        ...
                        >
                          <div
                            className={`w-8 h-8 sm:w-10 sm:h-10 rounded-xl flex flex-col items-center justify-center shrink-0 shadow-inner ${e.type === "TASK" ? "bg-[#F5990D] text-white" : "bg-[#4A7C59] text-white"}`}
                          >
                            <span
                              className={cn(
                                "text-[8px] sm:text-[9px] font-bold",
                                "leading-none opacity-80 mb-0.5",
                              )}
                            >
                          ...
                          <div className="flex-1 min-w-0">
                            <h4
                              className={cn(
                                ...
                                "leading-tight mb-0.5 truncate",
                              )}
                            >
                              {e.title}
                            </h4>
                            <div className="flex items-center justify-between">
                              <p className="text-[10px] font-bold text-[#84B0A5]">
                                {d.toLocaleDateString(
                                  "id-ID",
                                  {
                                    weekday:
              ...
              <div
                className={cn(
                  "shrink-0 pt-2.5 flex",
                  "justify-end mt-2 border-t",
                  "border-white/10 md:hidden",
                )}
              >
                <Link
                  href="/hub/calendar"
            ...
            <div
              className={cn(
                ...
                "md:w-[320px] shrink-0 flex",
                "flex-col justify-center md:h-full",
                "pb-1 pt-1 md:pt-0",
              )}
            >
              <div
                ...
                {(() => {
                  ...
                  for (
                    ...
                    cells.push(
                      <div
                        key={`empty-${i}`}
                        className="h-7 sm:h-8 rounded-lg opacity-10 bg-white/5"
                      ></div>,
                    );
                  }
                  ...
                  for (
                    ...
                    if (isToday) {
                      bgClass =
                        "bg-[#C25939] text-white font-black shadow-[0_4px_12px_rgba(194,89,57,0.4)] border-[#C25939]";
                    } else if (hasHarvest) {
                      bgClass =
                        "bg-white/20 text-white font-bold border-white/40";
                    } else if (hasEvent) {
                      bgClass =
                        "bg-[#4A7C59]/60 text-white font-bold border-[#4A7C59]";
                    }
                    ...
                    cells.push(
                      <Link
                        href="/hub/calendar"
                        key={d}
                        className={`h-7 sm:h-8 rounded-lg flex flex-col items-center justify-center text-[10px] font-bold transition-all relative ${bgClass}`}
                      >
                        {d}
                        {hasEvent &&
                          !isToday && (
                            <span
                              className={`absolute bottom-0.5 w-1 h-1 rounded-full ${hasHarvest ? "bg-[#C25939]" : "bg-white"}`}
                            ></span>
                          )}
                      </Link>,
                    );
              ...
              <div
                className={cn(
                  "shrink-0 pt-3 hidden",
                  "md:flex justify-end mt-2",
          ...
          >
            ...
            <div
              className={cn(
                ...
                "w-48 h-48 bg-emerald-300",
                "opacity-20 blur-[80px] rounded-full",
                "pointer-events-none",
              )}
            />
            ...
            <div className="p-4.5 sm:p-5 relative z-10 flex flex-col h-full">
              <div className="flex items-center justify-between mb-2">
                <img
                  src="/logos/intelligence/intelligence-white.webp"
                  alt="Pranata Intelligence"
                  className={cn(
                    "h-6 sm:h-8 w-auto",
                    "object-contain drop-shadow-md origin-left",
                  )}
                  loading="lazy"
                  decoding="async"
                />
                <button
                  onClick={
                    handleRefreshAIInsight
                  }
                  disabled={isLoading}
                  title="Refresh AI Insight (Minta analisis baru)"
                  className={cn(
                    "p-1.5 bg-white/10 hover:bg-white/20",
              ...
              <div className="flex-1 flex flex-col mt-1 overflow-hidden">
                ...
                events.length === 0 ? (
                  <div
                    className={cn(
                      "flex-1 flex flex-col",
                      "items-center justify-center bg-white/5",
                      "rounded-3xl border border-white/10",
                      "p-5 text-center backdrop-blur-md",
                    )}
                  >
                    <div
                      className={cn(
                    ...
                    <p
                      className={cn(
                        "text-[11px] sm:text-xs text-[#84B0A5]",
                        "leading-relaxed",
                      )}
                    >
                      Agen intelijen
                  ...
                  >
                    <Loader2
                      size={28}
                      className="text-[#F5990D] animate-spin mb-3"
                    />
                    <h4 className="font-black text-white text-base sm:text-lg mb-1">
                      Menganalisis Data
                    </h4>
                    <p
                      className={cn(
                  ...
                  <div className="flex-1 flex flex-col relative overflow-hidden">
                    ...
                    >
                      {(() => {
                        ...
                        if (
                          ...
                          const card1V =
                            lowStockProd
                              ? `${lowStockProd.title} (${lowStockProd.stock} Pcs)`
                              : pendingOrdersCount >
                                  0
                          ...
                          const card1D =
                            lowStockProd
                              ? `Stok ${lowStockProd.title} tersisa ${lowStockProd.stock} pcs. Segera restok.`
                              : pendingOrdersCount >
                                  0
                                ? `Ada ${pendingOrdersCount} pesanan aktif yang perlu diproses.`
                                : `Semua ${products.length} produk di etalase aktif.`;
                          const card1Text =
                            pendingOrdersCount >
                            0
                              ? "Proses Pesanan"
                          ...
                          const card2D =
                            upcomingEvt
                              ...
                              : temp &&
                                  temp > 30
                                ? `Suhu ${Math.round(temp)}°C tergolong tinggi. Pastikan pakan & air minum.`
                                : "Kelola rutinitas pakan dan kesehatan ternak hari ini.";
                            ...
                            (raw, idx) => {
                              ...
                              const desc =
                                descMatch
                                  ? descMatch[1]
                                      ...
                                      .trim()
                                  : idx === 0
                                    ? "Pantau pesanan peternakan Anda."
                                    : "Kelola rutinitas pakan ternak hari ini.";
                              const ctaText =
                                ctaTextMatch
                                  ? ctaTextMatch[1]
                                      .replace(
                              ...
                              return (
                                <div
                                  key={idx}
                                  className={cn(
                                    "bg-white/10 rounded-xl border",
                                    "border-white/20 p-2.5 sm:p-3",
                                    "md:p-4 backdrop-blur-md shadow-sm",
                                    "flex flex-col justify-between",
                                    "hover:bg-white/15 transition-all group",
                                    "md:flex-1 md:rounded-2xl md:bg-white/12",
                                    "md:border-white/25 md:shadow-md",
                                  )}
                                >
                                    ...
                                    <div className="mb-1 md:mb-2">
                                      <span
                                        className={cn(
                                          ...
                                          "text-[#A4C4A8] md:text-[#C8E6C9] uppercase",
                                          "tracking-wider md:bg-white/15 md:px-2.5",
                                          "md:py-0.5 md:rounded-full inline-block",
                    ...
                    <div
                      className={cn(
                        "shrink-0 pt-2.5 sm:pt-3",
                        "flex justify-end border-t",
                        "border-white/10 mt-2.5 sm:mt-3",
                      )}
                    >
                      <Link
                      ...
                      >
                        ...
                        <img
                          ...
                          alt="Pranata Intelligence"
                          className="h-4.5 sm:h-6 w-auto object-contain my-0.5"
                          loading="lazy"
                          decoding="async"
                        />
                        <ChevronRight
