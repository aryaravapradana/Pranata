...
export default function CalendarPage() {
  ...
  const [activeDateObj, setActiveDateObj] =
    useState(new Date());

  const [viewMode, setViewMode] =
    useState("Week"); // Month, Week, Day
  ...
  useEffect(() => {
    const timer = setTimeout(() => {
      ...
      if (
        viewMode === "Day" &&
        scrollRefDay.current
      ) {
        const dayEvents = events.filter(
          (e) =>
            e.day === activeDay &&
            e.month ===
              activeDateObj.getMonth() &&
            e.year ===
              activeDateObj.getFullYear(),
        );

        if (dayEvents.length > 0) {
          const validHours = dayEvents.map(
            (e) =>
              typeof e.hoursFromMidnight ===
              "number"
                ? e.hoursFromMidnight
                : 0,
          );
          ...
          scrollRefDay.current.scrollTop =
            targetTop;
      ...
      if (
        ...
        const weekDates = weekDays.map((w) =>
          w.fullDate.toDateString(),
        );
        ...
        if (weekEvents.length > 0) {
          const validHours = weekEvents.map(
            (e) =>
              typeof e.hoursFromMidnight ===
              "number"
                ? e.hoursFromMidnight
          ...
          scrollRefWeek.current.scrollTop =
            targetTop;
          scrollRefWeek.current.scrollTo({
            top: targetTop,
            behavior: "smooth",
          });
        } else {
  ...
  const fetchEvents = (id: string) => {
    fetchApi(`${API_BASE}/api/events/${id}`)
      .then((res) => res.json())
      .then((data) => {
        const eventsArray = Array.isArray(
          data,
        )
          ? data
        ...
        const uiEvents = eventsArray.map(
          (e: any) => {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (isEditing && formData.id) {
        res = await fetchApi(
          `${API_BASE}/api/events/${formData.id}`,
          {
            method: "PUT",
            headers: {
              "Content-Type":
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          {activeDateObj.toLocaleDateString(
            "id-ID",
            {
              month: "long",
              year: "numeric",
        ...
        <div
          className={cn(
            ...
            "md:justify-end gap-2.5 sm:gap-3",
            "md:gap-6 w-full md:w-auto",
          )}
        >
          {/* View Mode Switcher Pills */}
          <div
            className={cn(
              "bg-[#E8E3D2]/70 p-1 md:p-1.5",
              "rounded-full flex gap-1",
              "relative shadow-inner w-full",
              "sm:w-auto justify-between",
            )}
          >
            {["Month", "Week", "Day"].map(
              (view) => (
          ...
          >
            <button
              ...
              className={cn(
                "w-8 h-8 sm:w-10",
                "sm:h-10 bg-[#E8E3D2] hover:bg-[#DDE2D6]",
            ...
            <button
              onClick={handleToday}
              className={cn(
                "flex-1 sm:flex-none px-4",
                "sm:px-6 py-1.5 sm:py-2",
                "bg-[#E8E3D2] hover:bg-[#DDE2D6] rounded-full",
                "text-xs sm:text-sm font-bold",
                "text-[#2B4C3B] transition-colors text-center",
                "cursor-pointer",
              )}
            >
              Hari Ini
          ...
          >
            ...
            {viewMode === "Week" && (
              <div className="overflow-x-auto pb-3 mb-3 md:mb-4 hide-scrollbar">
                <div
                  className={cn(
                    "flex min-w-[540px] sm:min-w-[720px]",
                    "md:min-w-[880px]",
                  )}
                >
                  <div
                    ...
                    {weekDays.map((d) => (
                      <button
                        ...
                        className={`relative flex flex-col items-center justify-center py-2 sm:py-3.5 md:py-4 rounded-xl sm:rounded-2xl md:rounded-3xl transition-all cursor-pointer ${
                          ...
                          d.date
                            ? "text-white"
                            : "bg-white text-[#1C241E] border border-[#E8E3D2] hover:border-[#B4C179]"
                        }`}
                      >
                        {activeDay ===
                          d.date && (
                        ...
                        <span
                          className={`relative z-10 text-[10px] sm:text-xs md:text-sm font-bold mb-0.5 ${activeDay === d.date ? "text-[#A4C4A8]" : "text-[#7A8678]"}`}
                        >
                          {d.day}
                        </span>
                        <span
                          className={cn(
                            ...
                            "sm:text-2xl md:text-3xl font-black",
                          )}
                        >
                          {d.date}
                        </span>
            ...
            {viewMode === "Week" && (
              ...
              >
                <div
                  ...
                  className={cn(
                    "custom-scrollbar overflow-x-auto relative",
                    "shadow-inner",
                  )}
                >
                  <div
                    ...
                    className={cn(
                      "flex shrink-0 min-w-[640px]",
                      "sm:min-w-[720px] md:min-w-[880px] relative",
                    )}
                  >
                    {/* Time Column */}
                    <div
                      ...
                      className={cn(
                        ...
                        "pt-2 bg-[#F8F6F0] border-r",
                        "border-[#E8E3D2]/60 select-none",
                      )}
                    >
                      {HOURS_24.map(
                        (time) => (
                          <div
                            ...
                            className={cn(
                              ...
                              "pr-1.5 sm:pr-2 md:pr-3",
                              "text-[10px] sm:text-[11px] font-bold",
                              "text-[#7A8678] shrink-0",
                            )}
                          >
                    ...
                    <div
                      ...
                      className="flex-1 shrink-0 relative"
                    >
                      ...
                      <div
                        ...
                        className={cn(
                          ...
                          "flex-col pt-2 pointer-events-none",
                        )}
                      >
                        {HOURS_24.map(
                          (_, i) => (
                            <div
                              ...
                              className="border-t border-[#F8F6F0] w-full shrink-0"
                            ></div>
                          ),
                        )}
                      </div>
                          ...
                          (d, colIdx) => (
                            <div
                              ...
                              className={cn(
                                ...
                                "border-[#F8F6F0]/80 pointer-events-none",
                              )}
                            >
                                ...
                                (
                                  ...
                                  <div
                                    ...
                                    className={cn(
                                      ...
                                      "border-transparent hover:border-dashed hover:border-[#4A7C59]/60",
                                      "hover:bg-[#4A7C59]/10 rounded-xl transition-all",
                                      "cursor-pointer pointer-events-auto flex",
                                      "items-center justify-center group/slot",
                                      "z-0",
                                    )}
                                  >
                                    <div
                                      className={cn(
                                        ...
                                        "bg-[#4A7C59] text-white px-1.5",
                                        "py-0.5 rounded-lg text-[10px]",
            ...
            {viewMode === "Month" && (
              <div
                className={cn(
                  "bg-white rounded-2xl md:rounded-3xl",
                  "border border-[#E8E3D2] p-2.5",
                  "sm:p-5 md:p-6 shadow-sm",
                    ...
                    {[
                      ...
                      <div
                        ...
                        className={cn(
                          ...
                          "sm:text-xs md:text-sm text-[#1C241E]",
                          "bg-[#F8F6F0] py-1 sm:py-1.5",
                          "md:py-2 rounded-lg sm:rounded-xl",
                        )}
                      >
                        {d}
                      </div>
                  ...
                  <div className="grid grid-cols-7 gap-1 sm:gap-2 md:gap-3 lg:gap-4">
                    ...
                    {Array.from({
                      ...
                      <div
                        key={`empty-${i}`}
                        className={cn(
                          "min-h-[56px] sm:min-h-[80px] md:min-h-[110px]",
                          "lg:min-h-[120px] rounded-lg sm:rounded-xl",
                          "md:rounded-2xl p-1 sm:p-2",
                          "opacity-50 bg-[#F8F6F0]",
                        )}
                      ></div>
                    ))}
                    ...
                    {Array.from({
                      ...
                      return (
                        ...
                        >
                          <div
                            className={`mb-0.5 sm:mb-1 text-xs sm:text-base md:text-lg ${isToday ? "font-black text-[#C25939]" : "font-medium text-[#5A635B]"}`}
                          >
                            {day}
                          </div>
                          <div className="space-y-0.5 sm:space-y-1 relative z-10">
                            {dayEvents.map(
                              (e, idx) => (
                                <div
                                  key={idx}
                                  className={`text-[8px] sm:text-[9px] md:text-[10px] font-bold px-1 sm:px-1.5 md:px-2 py-0.5 sm:py-1 rounded sm:rounded-lg truncate ${e.color} shadow-2xs border`}
                                >
                                  {e.title}
                                </div>
                              ),
            ...
            {viewMode === "Day" && (
              ...
              >
                <div
                  className={cn(
                    "px-4 sm:px-8 py-4",
                    "border-b border-[#E8E3D2] bg-[#F8F6F0]",
                    "flex justify-between items-center",
                    "gap-2",
                  )}
                >
                  <div>
                    <h2 className="text-lg sm:text-2xl font-black text-[#1C241E]">
                      {activeDay}{" "}
                      {activeDateObj.toLocaleDateString(
                        "id-ID",
                        {
                    ...
                    <p
                      className={cn(
                        "text-[#5A635B] font-bold text-[11px]",
                        "sm:text-xs mt-0.5",
                      )}
                    >
                      {
                  ...
                  <button
                    ...
                    className={cn(
                      "bg-pranata hover:bg-[#1E362A] text-white",
                      "font-extrabold px-4 py-2.5",
                      "rounded-xl transition-all flex",
                      "items-center gap-1.5 text-xs",
                      "sm:text-sm shadow-md cursor-pointer",
                    )}
                  >
                    <Plus size={16} />{" "}
                    <span>
                ...
                >
                  <div
                    ...
                    className="flex shrink-0 bg-[#F8F6F0] p-3 sm:p-6 relative"
                  >
                    {/* Time Column */}
                    <div
                      style={{
                      ...
                      {HOURS_24.map(
                        (time) => (
                          <div
                            ...
                            className={cn(
                              "flex items-start justify-end",
                              "pr-2 md:pr-4 text-[10px]",
                              "sm:text-[11px] font-bold text-[#7A8678]",
                              "shrink-0",
                            )}
                          >
                            <span className="-mt-2.5">
                    ...
                    >
                        ...
                        {HOURS_24.map(
                          (_, i) => (
                            <div
                              ...
                              className={cn(
                                "border-b border-[#F8F6F0] w-full",
                                "last:border-0 shrink-0",
                              )}
                            ></div>
                          ),
                        )}
                      ...
                      <div
                        ...
                        className={cn(
                          ...
                          "sm:px-4 w-full md:w-3/4",
                          "lg:w-1/2 pt-2 pointer-events-none",
                        )}
                      >
                          ...
                          (_, hourIdx) => (
                            <div
                              ...
                              className={cn(
                                "absolute w-full border",
                                "border-transparent hover:border-dashed hover:border-[#4A7C59]/60",
                                "hover:bg-[#4A7C59]/10 rounded-2xl transition-all",
                                "cursor-pointer pointer-events-auto flex",
                                "items-center justify-center group/slot",
                                "z-0",
                              )}
                            >
                              <div
                                className={cn(
                                  "opacity-0 group-hover/slot:opacity-100 transition-opacity",
                                  "bg-[#4A7C59] text-white px-3",
                                  "py-1 rounded-xl text-xs",
                                  "font-black flex items-center",
                                  "gap-1 shadow-md",
                                )}
                              >
        ...
        {showModal && (
          <div
            className={cn(
              ...
              "p-4 bg-black/40 backdrop-blur-xs",
            )}
          >
            <motion.div
              initial={{
            ...
            >
              <button
                ...
                className={cn(
                  ...
                  "text-[#5A635B] hover:text-[#1C241E] cursor-pointer",
                )}
              >
                <X size={24} />
              </button>

              <h2 className="text-2xl font-black text-[#1C241E] mb-6">
                {isEditing
                  ? "Detail Jadwal"
                  : "Jadwal Baru"}
              </h2>
              ...
              <div className="space-y-4 mb-8">
                <div>
                  <label className="text-xs font-bold text-[#7A8678] mb-1 block">
                    Judul Kegiatan
                  </label>
                  <input
                    type="text"
                    ...
                    className={cn(
                      "w-full bg-[#F8F6F0] p-4",
                      "rounded-2xl font-bold focus:outline-none",
                      "focus:ring-2 focus:ring-[#4A7C59]",
                    )}
                    placeholder="Cek Stok Pakan Ternak"
                  />
                </div>
                  ...
                  <div className="sm:col-span-3 relative group">
                    ...
                    <div
                      className={cn(
                        ...
                        "bg-[#F8F6F0] border-2 border-transparent",
                        "group-hover:border-[#E8E3D2] focus-within:border-[#B4C179] focus-within:bg-white",
                    ...
                    >
                      <div
                        className={cn(
                          "w-9 h-9 sm:w-10",
                          "sm:h-10 flex items-center",
                          "justify-center bg-white rounded-xl",
                          "shadow-sm text-[#4A7C59] shrink-0",
                          "ml-1",
                        )}
                      >
                        <CalendarIcon
                      ...
                      <Popover>
                        <PopoverTrigger
                          className={cn(
                            ...
                            "px-3 font-bold text-[#1C241E]",
                            "focus:outline-none text-left text-xs",
                            "sm:text-sm truncate",
                          )}
                        >
                          {formData.eventDate
                ...
                <div>
                  <label className="text-xs font-bold text-[#7A8678] mb-1.5 block">
                    Tipe Kegiatan
                  </label>
                  ...
                  {selectedCategory ===
                    ...
                    <motion.div
                      ...
                      className="mt-3"
                    >
                      <label className="text-[11px] font-bold text-[#7A8678] mb-1 block">
                        Nama Tag Custom
                      </label>
                      <input
                        type="text"
              ...
              <div className="flex gap-3 mt-4">
                {isEditing && (
                  <button
                    ...
                    className={cn(
                      "w-16 bg-rose-50 border",
                      "border-rose-100 hover:bg-rose-100 text-rose-500",
                      "flex items-center justify-center",
                ...
                <button
                  ...
                  onClick={handleSave}
                  className={cn(
                    "flex-1 bg-pranata disabled:opacity-75",
                    "disabled:cursor-not-allowed text-white py-4",
...
function EventCard({
  ...
  const topPixels = Math.max(
    0,
    (event.hoursFromMidnight ?? 0) *
      ROW_HEIGHT +
      8,
  );
  const heightPixels = Math.max(
    64,
    (event.durationHours ?? 1) * ROW_HEIGHT -
      6,
  );
  const style =
    event.style || getEventStyle(event.type);
    ...
    >
      <h3
        className={cn(
          ...
          "leading-tight mb-1 break-words",
          "line-clamp-2",
        )}
      >
        {event.title}
      ...
      <div className="mt-auto pt-1 flex items-center">
        <span
          className={`px-2 py-0.5 rounded-full text-[9px] sm:text-[10px] font-black uppercase tracking-wider ${style.badgeBg} ${style.badgeText} shadow-xs truncate`}
        >
          {style.label}
        </span>
      </div>
